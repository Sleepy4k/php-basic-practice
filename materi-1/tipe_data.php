<?php

/*
|--------------------------------------------------------------------------
| Tipe Data
|--------------------------------------------------------------------------
|
| Contoh tipe data yaitu :
|   - boolean
|   - integer
|   - float
|   - string
|
| 1. Boolean
|   - Pengertian
|       Menyimpan data bernilai true atau false. Atau dapat ditulis dengan
|       angka 1 atau 0.

|   - Contoh
|       ^ true
|       ^ false
|       ^ 1
|       ^ 0
|
| 2. Integer
|   - Pengertian
|       Menyimpan data bilangan bulat, baik berupa bilangan desimal, oktal
|       maupun hexadesimal. Jangkauan nilai integer adalah -2^31 sampai 2^31.

|   - Contoh
|       ^ 45 (desimal)
|       ^ -65446 (desimal)
|       ^ 0755 (oktal)
|       ^ 0xC4E (hexadesimal)
|
| 3. Float
|   - Pengertian
|       Menyimpan data bilangan yang mengandung pecahan (berkoma). Biasanya
|       untuk menyimpan nilai mata uang, berat, atau jarak.
|
|   - Contoh
|       ^ 4.9876
|       ^ 4.0
|       ^ 7.5e4
|       ^ 1.34E+11
|
| 4. String
|   - Pengertian
|       Menyimpan data berupa teks (karakter). Teks harus diapit oleh tanda
|       kutip satu atau kutip dua.
|
|   - Contoh
|       ^ "Belajar PHP"
|       ^ "Learning-by-doing"
|       ^ "Jl. Kemuning No. 7"
|       ^ "123$%^456"
|
*/

?>